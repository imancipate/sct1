...
class WPCF_Field
{
    ...
    function set( $post, $cf ) {
        ...
        if ( is_string( $cf ) ) {
            ...
            $cf = wpcf_admin_fields_get_field( $this->selfcare__get_slug_no_prefix( $cf ) );
            if ( empty( $cf ) ) {
                /*
...
//                        'Since Types 1.2 we encourage developers to completely define fields' );
    ...
    function is_under_control( $field_key ) {
         ...
         */
        $key = $this->selfcare__get_slug_no_prefix( $field_key );
     ...
     * @return type
     */
    function selfcare__get_slug_no_prefix( $field_key ) {
