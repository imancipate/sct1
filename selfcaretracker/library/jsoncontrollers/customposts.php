<?php
/*
Controller name: customposts
Controller description: Data manipulation methods for scr_custom posts
*/

class JSON_API_customposts_Controller {
...
//print_r($current_user->caps);
    ...
    if (!current_user_can('edit_scr_custom_posts')) {
      $json_api->error("You need to login with a user capable of creating scr posts.");
    }
    if (!$json_api->query->nonce) {
      $json_api->error("You must include a 'nonce' value to create posts. Use the `get_nonce` Core API method.");
    }
    if ($json_api->query->author && ($json_api->query->author != $current_user->user_login) && !current_user_can('edit_others_scr_custom_posts')) {
      
	  $json_api->error("You cannot post for another user : ".$json_api->query->author);
    }
    $nonce_id = $json_api->get_nonce_id('posts', 'create_post');
    if (!wp_verify_nonce($json_api->query->nonce, $nonce_id)) {
      $json_api->error("Your 'nonce' value was incorrect. Use the 'get_nonce' API method or visit this link: /selfcare/api/core/get_nonce?controller=posts&method=create_post");
    }
    nocache_headers();
    $post = new JSON_API_Custom_Post();
...
class JSON_API_Custom_Post {
  ...
  var $scr_categories;      // Array of objects
  ...
  function save($values = null) {
    ...
    if (!empty($values['excerpt'])) {
      $wp_values['post_excerpt'] = $values['excerpt'];
    }
 ...
 // echo "<pre>".print_r($wp_values, true)."</pre>";

	if (isset($values['scr_categories'])) {
		$scr_categories = explode(',', $values['scr_categories']);
		$post_terms = wp_set_post_terms( $this->id, $values['scr_categories'], 'scr_category', true );
		//echo "<pre>".print_r($post_terms , true)."</pre>";
	}
  ...
  function set_categories_value() {
    ...
    function set_scr_categories_value() {

	  $this->scr_categories[] = $values['scr_categories'];
