...
class JSON_API_scrcustom_Controller {
	  ...
	  public function get_author_posts() {
		...
		/*
		echo '<pre>'.print_r($author, true).'</pre>'; exit;

		if($current_user->ID!=$author && !current_user_can('edit_users')){ //not authorized
			echo 'Not authorized to view this resource';
			exit;
		}
		*/
		...
		//echo '<pre>'.print_r($author_id, true).'</pre>'; 
		//echo '<pre>'.print_r($post_status, true).'</pre>'; 
		
		
		if (!$author || ($current_user->id!=$author_id && $author->id != 1 && $post_status == 'publish')) {
		  $json_api->error("No author specified or lacking viewing permissions2.$author_id.$current_user->id.$post_status");
		}
		...
		//echo '<pre>'.print_r($jpost, true).'</pre>'; //exit;
			$this->add_taxonomies( $jpost );
			$this->add_taxonomy_ids( $jpost );
			$this->filter_private( $jpost );
			$jpost->content = strip_tags($jpost->content);
			$jpost->excerpt = strip_tags($jpost->excerpt);
		}
    ...
    protected function add_taxonomy_ids( $post ) {
        global $wp_query;
		//echo '<pre>'.print_r($wp_query,true).'</pre>'; exit;
    ...
    protected function get_term_by_id($term_id, $taxonomy) {
        ...
        if ( !$term ) return null;
        return new JSON_API_Category( $term );
    }
